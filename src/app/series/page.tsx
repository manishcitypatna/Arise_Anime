...
export default function SeriesPage() {
  ...
  const [typeFilter, setTypeFilter] = useState<'all' | 'movies' | 'series'>('series');
  ...
  if (sortBy === 'Name') {
    ...
    filteredSeries = [...filteredSeries].sort((a, b) => b.year - a.year);
  }
    ...
    <div className="min-h-screen flex flex-col bg-[#0f0f1f] text-white">
      ...
      <main className="flex-grow pt-16 pb-10 container mx-auto px-4 sm:px-6 lg:px-8 flex gap-4">
        <div className="flex-1 min-w-0 flex flex-col">
          <h1 className="text-3xl font-bold font-headline text-foreground mb-2 mt-4">Series</h1>
          <div className="text-sm text-muted-foreground mb-6">Home / Series</div>
            ...
            <section className="flex-1 min-w-0 flex flex-col mt-8">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 lg:grid-cols-7 gap-4 sm:gap-6">
                {paginatedSeries.map((series, idx) => (
                  <div key={series.id + idx} className="group/moviecard block w-full shrink-0">
              ...
              <div className="flex flex-col sm:flex-row items-center justify-between mt-8 gap-4">
                <div>
                  <label htmlFor="seriesPerPage" className="mr-2 text-sm font-body text-muted-foreground">Series per Page</label>
                  <select id="seriesPerPage" value={pageSize} onChange={e => { setPageSize(Number(e.target.value)); setCurrentPage(1); }} className="bg-muted text-foreground rounded px-2 py-1 font-body text-sm border border-border">
                    {PAGE_SIZES.map(size => (
